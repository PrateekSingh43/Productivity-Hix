...
describe("Baseline: Engine", () => {
  ...
  const dummyPopulation: DummySession[] = [
    ...
    { id: "4", timestamp: "2023-01-02T12:00:00Z", switches: -1, valid: false }, // Should be filtered out
  ];
  ...
  it("should successfully evaluate a valid baseline", () => {
    ...
    assert.strictEqual(result.qualifiedPopulationCount, 3); // item 4 filtered out
    assert.strictEqual(result.distinctCalendarDayCount, 2); // Jan 1, Jan 2
    
    // Median of [5, 10, 15] is 10
    assert.strictEqual(result.aggregatedValue, 10);
  });
  ...
  it("should return INSUFFICIENT_BASELINE_DATA if population count is below minimum", () => {
    const config = { ...defaultConfig, minimumPopulationCount: 4 };
